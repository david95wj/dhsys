...
public class SysModuleDao extends IHibernateDao<TModule>{
	...
	public List getModuleTreeDao(long adminFlag){
	...
	public TModule getModuleByIdDao(Integer moduleId){
		TModule module = null;
		try{
			module = (TModule) this.getHibernateDao().find("From TModule m where m.moduleId = " + moduleId).get(0);
		}catch(HibernateException e){
			e.printStackTrace();
		}
		return module;
